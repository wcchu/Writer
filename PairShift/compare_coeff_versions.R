...
ggsave(p_hist, filename = 'p_hist.png', height = 10, width = 15, units = 'cm')
...
ggsave(p_diff, filename = 'p_diff.png', height = 10, width = 15, units = 'cm')
...
p_hist_existence <-
  ...
  geom_histogram(data = t %>% filter(!is.na(v2)),
                 aes(x = v2), binwidth = 0.025, alpha = 0.5, fill = 'red') +
  facet_grid(existence ~ ., scales = "free") +
